$(document).ready(function() {
    ...
    $.ajax({
      ...
      for (var i = 0; i < TMData._embedded.events.length; i++) {
        if (TMData._embedded.events[i]._embedded.venues[0].state != undefined) {
          var eventState =
            TMData._embedded.events[i]._embedded.venues[0].state.stateCode;
        } else {
          var eventState = "n/a";
        }
        var eventCity =
          TMData._embedded.events[i]._embedded.venues[0].city.name;
        ...
        var ticketData =
          "<a href=" +
          TMData._embedded.events[i].url +
          "span class='buy-tag'>Buy Tickets!</a>";
  ...
  function runItunes(queryURL) {
    ...
    $.ajax({
      ...
      for (let i = 0; i < 10; i++) {
        ...
        $("#ItunesSection" + i).append(
          "<h1>" + response.results[i].trackName + "<h1>"
        );
        $("#ItunesSection" + i).append(
          "<a href=" +
            response.results[i].trackViewUrl +
            "span class='song-preview'> Preview Song <br> </a>"
        );
        $("#ItunesSection" + i).append(
          "<a href=" +
            response.results[i].artistViewUrl +
            "span class='itunes-page'> Artist Itunes Page </a>"
        );
      }
    });
  }
  ...
  $("#submit-btn").on("click", function() {
    ...
    if (parseInt(startDate && endDate)) {
      ...
      newURL =
        newURL + "*&startDateTime=" + startDate + "&endDateTime=" + endDate;
    }
